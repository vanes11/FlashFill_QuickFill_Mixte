...
class InteractQuickFill:
    ...
    def ExpressionConcatenate(self,entree,s,id):
        ...
        Test =  FinalExpression.findall(entree) # Test est une liste

        if s in Test:
            position1  = Test.index(s)
            position2 = -(len(Test)-position1)               
            ExpressionExecuteFormule1 = "SubStr2(" + id + "," + "TokenSeq(" + Tokenname + ")," + str(position1+1) + ")"
            ExpressionExecuteFormule2 = "SubStr2(" + id + "," + "TokenSeq(" + Tokenname + ")," + str(position2) + ")"
