...
class UniformQuickFill:
    ...
    def  GenerateStr3(self,entree, s):
        ...
        for i in PsiTilda:                                # Pour chaque arete
            ...
            Substr2 = self.ExpressionConcatenate(entree,s[i[0]:i[1]])
            ConstString = ConstString.union(SubString) 
            ConstString = ConstString.union(Substr2)
    ...
    def JoinAllElements(self,ListofConcatenate):

        ListofConcatenateNew = []
        ListofConcatenateStep = []
        ListeDesExpression = []
        for elt in ListofConcatenate:
            ListofConcatenateStep = []
            for elt2 in elt:
                elt2 = elt2.replace("Concatenate(", "")
                original_string = elt2
                last_char_index = original_string.rfind(")")
                elt2 = original_string[:last_char_index]
                ListofConcatenateStep.append(elt2)
            
            ListofConcatenateNew.append(ListofConcatenateStep)
            

        ToutesLesCombinaissons = list(itertools.product(*ListofConcatenateNew))
    ...
    def ExpressionConcatenate(self,entree,s):
        ...
        k1 = 0
        Tokens = []
        ExpressionExecuteFormule = set ()
       
        k=len(s)

        while  k1 < k :  # on cherche l'expression reguliere de la sortie
            for cle in self.ClasseC:
                Token = self.ClasseC[cle]
                TokenComp = re.compile(Token)
        ...
        FinalExpression = ''.join([elt[1] for elt in Tokens])  # expression reguliere du parametre s    
        Tokenname = ','.join([elt[2] for elt in Tokens])
        

        FinalExpression = re.compile(FinalExpression)     
            
        for key in entree:
           
            Test =  FinalExpression.findall(entree[key]) # Test est une liste

            if s in Test:
                position1  = Test.index(s)
                position2 = -(len(Test)-position1)               
                ExpressionExecuteFormule1 = "SubStr2(" + key + "," + "TokenSeq(" + Tokenname + ")," + str(position1+1) + ")"
                ExpressionExecuteFormule2 = "SubStr2(" + key + "," + "TokenSeq(" + Tokenname + ")," + str(position2) + ")"
                ExpressionExecuteFormule = ExpressionExecuteFormule.union(set([ExpressionExecuteFormule1]))
                ExpressionExecuteFormule = ExpressionExecuteFormule.union(set([ExpressionExecuteFormule2]))
    ...
    def GenerateStringProgram3(self,S):
        ...
        for elt in S:
            Result = self.GenerateStr3(elt[0],elt[1])
