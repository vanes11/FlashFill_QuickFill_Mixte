...
class FlashFillExecutionFreeLoopList(APIView):    
    ...
    def post(self, request, format=None):
        ...
        if len(DataExperience) != 0:
            ...
            datas["memoryFlasfill"] = sys.getsizeof(ListeOfProgrammes)/1024   
            time.sleep(1)
            end = time.time()
        ...
        if NombreExemples >  1000000:
            if random_index > 1000000:
                datas["listedesprogrammes"] = ListeOfProgrammes[:999999]
                datas["listedesprogrammes"].append(ListeOfProgrammes[random_index])
                datas["indiceduprogrammechoisi"] = 1000000
            else:
